...
Models with their own geometry format keep `to_sample` and swap the builder: `to_sample(atoms, cutoff, structure_fn=my_prepare)` calls `my_prepare(atoms, cutoff, float_dtype=..., int_dtype=...)` instead of `to_structure` and still merges inputs and reads labels. `ToSample` takes the same `structure_fn` (a `functools.partial` of a module-level function, so grain workers can pickle it).

...
def test():
    lj = LennardJones(
        cutoff=rc, onset=ro, initial_sigma=sigma, initial_epsilon=epsilon, ensemble=True
    )
    ...
    pred_fn = get_predict_fn(lj.apply, stress=True, derivative_variance=True)
    ...
    loss_fn = get_loss_fn(
        ...
        loss_functions={key: "crps" for key in keys},
    )
